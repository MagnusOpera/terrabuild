...
let rec eval (context: EvaluationContext) (expr: Expr) =
    let rec eval (location: SourceLocation option) (expr: Expr) =
        let resolvedLocation, expression =
            match expr with
            | Expr.Located (exprLocation, value) -> Some exprLocation, value
            | _ -> location, expr

        let rethrowWithLocation (ex: exn) =
            match resolvedLocation with
            | Some location -> withLocation location ex |> raise
            | None -> raise ex
        ...
        try
            ...
            | Expr.Map map ->
                let values =
                    ...
                    |> Map.fold (fun map k v ->
                        let mv = eval resolvedLocation v
            ...
            | Expr.Function (f, exprs) ->
                let values =
                    exprs
                    |> List.fold (fun exprs expr ->
                        let e = eval resolvedLocation expr
                        exprs @ [e]) []
                let res =
                    match f, values with
                ...
                res
            | Expr.Located _ ->
                raiseBugError "Unexpected located expression node during evaluation"
        with
        | ex -> rethrowWithLocation ex
