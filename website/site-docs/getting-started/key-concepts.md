...
Target attributes form the scheduling and reuse policy for every task with that name. See [Target policies](./target-policies) for dependency, phase, build, output, artifact, batch, environment, and lock decisions.
...
- [Target policies](./target-policies) for choosing target attributes from use cases
